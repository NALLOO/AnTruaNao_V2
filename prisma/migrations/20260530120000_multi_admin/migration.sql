-- Multi-admin: backfill dữ liệu cũ, không xóa weeks/orders

-- Step 1: Cột profile admin (nullable trước)
...
-- Step 2: Week.adminId nullable trước
ALTER TABLE "weeks" ADD COLUMN IF NOT EXISTS "adminId" TEXT;

-- Step 3: Đảm bảo có ít nhất 1 admin + backfill profile
...
-- Tránh trùng slug trước unique index
UPDATE "admins" a
...
-- Step 4: Gán mọi tuần cũ cho admin tạo sớm nhất
UPDATE "weeks"
